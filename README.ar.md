...
PengyuyanPPT_Skill هو Codex Skill لتحويل المستندات ومواد البحث وبيانات الأعمال إلى عروض PowerPoint عالية الكثافة وقابلة للتحرير وبأسلوب استشاري.
...
PengyuyanPPT_Skill ليس قالبا جاهزا فقط. إنه يحول المصادر إلى سلسلة أدلة قابلة للتدقيق، ثم يستخدم منطق SCR، وتخطيط كثافة الصفحات، والمخططات البصرية، وبوابات صارمة لإنتاج ملفات PPTX قابلة للتحرير وعالية الوفاء بصريا.
...
- ارفع مستنداتك، ثم أخبر Codex بوضوح: «استخدم Skill ‏PengyuyanPPT_Skill الموجود في المجلد XX لإنشاء عرض PPT استنادًا إلى المستندات/المواد المرفوعة»، مع إضافة أي متطلبات أخرى. في هذه المرحلة، يحلل PengyuyanPPT_Skill المواد ويُنشئ مسودة موثقة بالأدلة.

- ملاحظة: يحلل PengyuyanPPT_Skill المواد تلقائيًا ويحدد عدد الشرائح المطلوب. ويمكنك أيضًا تحديد عدد الشرائح بنفسك عند الحاجة.

**2. اختيار النمط وإنشاء المخططات الأولية.**

- يعرض PengyuyanPPT_Skill ثمانية أنماط مدمجة للاختيار من بينها. بعد اختيار النمط، ينتقل إلى مرحلة إنشاء المخططات الأولية وينشئ مخططات جميع الشرائح دفعة واحدة. لا تكون هذه المخططات قابلة للتحرير في هذه المرحلة.

**3. إنشاء عرض PPT قابل للتحرير.**

- في هذه المرحلة، يعيد PengyuyanPPT_Skill بناء المخططات الأولية شريحةً تلو الأخرى. قد يتشتت انتباه الذكاء الاصطناعي؛ فإذا بدأ بإعادة بناء جميع الشرائح دفعة واحدة، أوقفه فورًا وقل له: «اتبع الـ Skill بدقة وأعد بناء الشرائح واحدةً تلو الأخرى». وإلا فقد يختصر العمل وينتج هياكل تخطيطية فقط. إذا انحرفت شريحة عن مخططها الأولي، فأشر إلى المشكلة فور ظهورها (**تعمل لقطات الشاشة المشروحة بأسهم بأفضل صورة**). تستغرق الشريحتان الأولى والثانية عادةً وقتًا أطول وتكونان أكثر عرضة للأخطاء. يصعب تجنب هذه المعايرة في التشغيل الأول، ولكن بعد ضبط سير العمل سيصبح PengyuyanPPT_Skill أسرع وتتحسن نتائجه تدريجيًا.
...
- توفير 8 أنماط بصرية ثابتة من PengyuyanPPT_Skill، ولكل نمط عينة مستقلة بنسبة 16:9.
...
يتضمن PengyuyanPPT_Skill عدة بوابات صارمة لمنع إنتاج عروض تبدو مكتملة لكنها تفشل في الأدلة أو الكثافة أو قابلية التحرير أو الوفاء البصري.
...
استخدم Git لتثبيت PengyuyanPPT_Skill داخل مجلد Codex skills واحتفظ باسم التثبيت `pengyuyan-ppt-skill`. يجب أن يحتوي الجذر على `SKILL.md`.
